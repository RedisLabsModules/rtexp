...
ElementQueueNode* NewNode(char* element, size_t element_len, mstime_t ttl)
{
    ElementQueueNode* newNode
        = (ElementQueueNode*)rm_malloc(sizeof(ElementQueueNode));
    newNode->element = rm_strndup(element, element_len);
...
ElementQueue* newQueue() {
    ElementQueue* queue
        = (ElementQueue*)rm_malloc(sizeof(ElementQueue));
    queue->head = NULL;
    queue->tail = NULL;
    queue->len = 0;
    return queue;
...
Lawn* newLawn(void)
{
    ...
    Lawn* lawn
        = (Lawn*)rm_malloc(sizeof(Lawn));

    lawn->element_count = 0;
...
size_t timer_count(Lawn* lawn){
    return lawn->element_count--;//lawn->element_nodes->cardinality;
}
...
ElementQueueNode* pop_next(Lawn* lawn) {
    ...
    if (next_node != NULL) {
        ...
        ElementQueue* queue = TrieMap_Find(lawn->timeout_queues, ttl_str, strlen(ttl_str));
        char* retval = rm_strndup(next_node->element, next_node->element_len);
